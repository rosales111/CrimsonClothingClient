$(document).ready(function () {
  ...
  clothingItems.forEach((item, index) => {
    ...
    // Create occasion element
    const occasionP = document.createElement("p");
    occasionP.textContent = item.occasion;
    ...
    infoWrapper.appendChild(typeP);
    infoWrapper.appendChild(occasionP);
    infoWrapper.appendChild(sizeP);
    infoWrapper.appendChild(priceP);

    // Append img and info wrapper to card
    ...
    // Append card container to clothing container
    ...
    removeBtn.addEventListener("click", function () {
      ...
      console.log(index);
      console.log(clothingItems[index]);
      clothingItems.splice(index, 1);

      // Update local storage with the new array
      localStorage.setItem("cart", JSON.stringify(clothingItems));

      // Remove card container from the DOM
